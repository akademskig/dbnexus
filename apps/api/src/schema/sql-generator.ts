...
/**
 * Convert a column with serial/identity default to use appropriate auto-increment syntax
 * Returns the column definition string
 */
function getColumnDefForCreate(col: ColumnInfo, engine: DatabaseEngine): string {
    const quotedName = quoteIdentifier(col.name, engine);

    // Check if this is an auto-increment/serial/identity column
    if (isIdentityOrSerialDefault(col.defaultValue)) {
        if (engine === 'postgres') {
            // Use GENERATED AS IDENTITY (modern PostgreSQL syntax)
            // Determine if it's bigint or regular int based on the data type
            ...
            let def = `${quotedName} ${baseType} AUTO_INCREMENT`;
            if (!col.nullable) def += ' NOT NULL';
            return def;
        } else {
            // SQLite: INTEGER PRIMARY KEY auto-increments automatically
            // The PRIMARY KEY will be added separately if needed
...
export function generateAddColumnSql(
    ...
    if (isIdentityOrSerialDefault(col.defaultValue)) {
        if (engine === 'postgres') {
            // For PostgreSQL, adding a serial/identity column requires special handling
            const isBigInt = col.dataType.toLowerCase().includes('big');
            const baseType = isBigInt ? 'BIGINT' : 'INTEGER';
            ...
            const isBigInt = col.dataType.toLowerCase().includes('big');
            const baseType = isBigInt ? 'BIGINT' : 'INT';
            let sql = `${prefix} ${quoteIdentifier(col.name, engine)} ${baseType} AUTO_INCREMENT`;
