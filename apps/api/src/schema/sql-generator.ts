...
export function generateCreateTableSql(table: TableSchema, engine: DatabaseEngine): string[] {
    ...
    if (primaryKey.length > 0) {
        columnDefs.push(`PRIMARY KEY (${primaryKey.map((c) => quoteIdentifier(c, engine)).join(', ')})`);
    }
 ...
 */
export function generateDropTableSql(schema: string, table: string, engine: DatabaseEngine): string {
...
function generateAlterColumnTypeSql(
    ...
    if (engine === 'postgres') {
        ...
        return [`-- SQLite: Cannot alter column type. Manual migration required for "${source.name}"`];
    }
}
...
function generateAlterColumnNullableSql(
    ...
    if (engine === 'postgres') {
        if (source.nullable) {
            return [`${prefix} ALTER COLUMN ${quoteIdentifier(source.name, engine)} DROP NOT NULL;`];
        } else {
        ...
        return [`-- SQLite: Cannot alter column nullability. Manual migration required for "${source.name}"`];
    }
}
...
function generateAlterColumnDefaultSql(
    ...
    if (engine === 'postgres' || engine === 'mysql' || engine === 'mariadb') {
        if (source.defaultValue === null) {
            ...
            return [`${prefix} ALTER COLUMN ${quoteIdentifier(source.name, engine)} SET DEFAULT ${source.defaultValue};`];
        }
    } else {
        return [`-- SQLite: Cannot alter column default. Manual migration required for "${source.name}"`];
    }
}
