...
export class SyncService {
    ...
    async dumpAndRestore(
        ...
        this.logger.log(`Dump & Restore: Processing ${tables.length} tables in order: ${tables.join(', ')}`);
        ...
        try {
            ...
            for (const table of tables) {
                ...
                try {
                    ...
                    const sourceData = await sourceConnector.query(`SELECT * FROM ${sourceTableRef}`);
                    ...
                    for (let i = 0; i < sourceData.rows.length; i += batchSize) {
                        ...
                        for (const row of batch) {
                            try {
                                ...
                                const placeholders = columns.map((_, idx) => getPlaceholder(idx + 1, targetEngine)).join(', ');
                                const quotedColumns = columns.map(c => quoteIdentifier(c, targetEngine)).join(', ');
                                ...
                                this.logger.warn(`Failed to insert row in ${table}: ${insertError}`);
                            }
                        }
                    }
